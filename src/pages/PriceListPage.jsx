...
const formatRate = (amount) => new Intl.NumberFormat('es-AR', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(amount || 0);
...
const ProductRow = ({ product }) => (
  <tr className="border-b border-gray-200 hover:bg-gray-50">
    ...
    <td className="py-3 px-4 text-sm text-gray-600 font-medium text-right">
      {(Number(product.moneda) === 2 || Number(product.moneda) === 3) ? formatUSD(product.originalPrice) : '-'}
    </td>
...
const ErrorMessage = ({ message, onRetry, showRetry = true }) => (
  <div className="text-center p-6 bg-white rounded-lg shadow-md my-6">
    ...
    {showRetry && onRetry && (
      <button onClick={onRetry} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        Volver a intentar
      </button>
    )}
  </div>
...
export default function PriceListPage({ onNavigate }) {
  ...
  const pdfMutation = useMutation({
    ...
    onSuccess: (products) => {
      ...
      doc.setFontSize(10);
      const brandText = selectedBrands.length > 0 ? selectedBrands.join(', ') : 'Todas';
      doc.text(`Filtros: ${debounceSearchTerm || 'Ninguno'} | Marca: ${brandText}`, 14, 28);
      doc.text(`Fecha: ${new Date().toLocaleDateString('es-AR')}`, doc.internal.pageSize.getWidth() - 14, 28, { align: 'right' });

      const columns = ["Código", "Descripción", "Marca", "Grupo", "Mon", "Cotiz", "Precio USD", "Precio ARS"];
      const rows = products.map(p => [
        p.code, p.name, p.brand, p.product_group,
        formatMoneda(p.moneda), formatRate(p.cotizacion),
        (Number(p.moneda) === 2 || Number(p.moneda) === 3) ? formatUSD(p.originalPrice) : '-',
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <header className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          <button onClick={() => onNavigate('dashboard')} className="flex items-center justify-center p-2 mr-4 text-gray-600 bg-white rounded-full shadow-md hover:bg-gray-100 transition-colors" aria-label="Volver al dashboard">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <h1 className="text-3xl font-bold text-gray-800">Lista de Precios</h1>
        </div>
        <button onClick={handleGeneratePDF} disabled={pdfMutation.isPending} className="inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 transition-colors disabled:opacity-50">
          {pdfMutation.isPending ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Download className="w-5 h-5 mr-2" />}
          Descargar PDF
        </button>
      </header>
          ...
          {isBrandDropdownOpen && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {brandsData && brandsData.map(brand => (
                <label key={brand} className="flex items-center px-4 py-2 hover:bg-gray-100 cursor-pointer">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white">
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading && allProducts.length === 0 && Array.from({ length: 10 }).map((_, i) => <ProductRowSkeleton key={i} />)}
              {isError && (
                <tr>
                  <td colSpan="8"><ErrorMessage message={error.message} onRetry={() => window.location.reload()} /></td>
                </tr>
              )}
              {allProducts.length > 0 && allProducts.map((product) => <ProductRow key={product.id} product={product} />)}
            </tbody>
          </table>
        </div>
        
        <div className="p-4 text-center">
          {!isLoading && !isError && allProducts.length === 0 && <p className="text-gray-500">No se encontraron productos con esos filtros.</p>}
          {isFetchingNextPage && (
          ...
          {hasNextPage && (
            <button onClick={() => fetchNextPage()} disabled={isFetchingNextPage} className="mt-4 px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center justify-center mx-auto">
              {isFetchingNextPage ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Cargando más...</> : 'Cargar más productos'}
            </button>
          )}
        </div>
      </div>
