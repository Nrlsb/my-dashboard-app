...
fs.createReadStream(filePath)
  ...
  .on('end', async () => {
    console.log(
      `Lectura de CSV completada. Se encontraron ${groupToBrandMap.length} mapeos de Grupo a Marca.`
    );
    ...
    try {
      ...
      console.log(
        'Transacción de actualización de marcas (por grupo) iniciada.'
      );
      ...
      for (const row of groupToBrandMap) {
        ...
        if (!groupCode) {
          console.warn(
            `Fila ${rowCounter} omitida: El 'Cod. Grupo' está vacío o es nulo. Datos:`,
            row
          );
          failedGroups++;
        ...
        if (!brand) {
          console.warn(
            `Fila ${rowCounter} (Grupo: ${groupCode}) omitida: La 'Marca' está vacía o es nula.`
          );
          failedGroups++;
          continue; // Omitir esta fila
        }
        ...
        try {
          ...
          if (result.rowCount > 0) {
            ...
            console.log(
              `Grupo ${groupCode} ('${brand}'): ${result.rowCount} productos actualizados.`
            );
          } else {
            // Esto es normal si no hay productos de ese grupo
            console.warn(
              `Fila ${rowCounter} (Grupo: ${groupCode}): No se encontraron productos con ese grupo para actualizar.`
            );
            failedGroups++;
          }
        } catch (updateError) {
      ...
      console.log(
        `Resultados: ${updatedGroups} grupos procesados, ${affectedRows} productos en total actualizados, ${failedGroups} grupos omitidos/no encontrados.`
      );
    } catch (error) {
