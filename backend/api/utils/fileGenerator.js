...
async function generateOrderPDF(orderData) {
  try {
    ...
    page.drawText(`Pedido N°: ${newOrder.id}`, {
      x: margin,
      y: y,
      font: boldFont,
      size: 12,
    ...
    page.drawText(
      `Fecha: ${new Date(newOrder.created_at).toLocaleDateString('es-AR')}`,
      { x: width / 2, y: y, font: font, size: 12 }
    );
    y -= lineHeigh;
    page.drawText(`Cliente: ${user.full_name} (Cód: ${user.a1_cod})`, {
      x: margin,
      y: y,
      font: font,
      size: 12,
